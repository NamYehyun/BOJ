...
int main(void) {
    ...
    string st = s + "$" + t;
    const int length = s_length + 1 + t_length;

    vector<int> suffix_array;
    vector<int> lcp_array;

    set_suffix_array(st, suffix_array);
    set_lcp_array(st, suffix_array, lcp_array);

    int pos = 0;
    int ans = 0;
    for (int i = 1; i < length; ++i) {
        if ((suffix_array[i-1] < s_length && suffix_array[i] > s_length)
            || (suffix_array[i-1] > s_length && suffix_array[i] < s_length)) {
            if (ans < lcp_array[i]) {
                ans = lcp_array[i];
                pos = suffix_array[i];
            }
